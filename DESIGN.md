...
**Roster (presentation-only names).** The opening `chat_started` control may carry `roster: [{id, display_name}]`. This exists so a human reading a transcript sees `mini-moss (juno)` rather than a bare wire id, and two participants that both call themselves "moss" stay distinguishable. The split is deliberate and load-bearing: the relay attests `author` as the token-map key, and alternation keys on `participants`, so **display names never enter auth or validation** — a name is a label, not a second identity. A malformed roster entry is dropped rather than fatal, because a decode failure would null the whole control (`bad_shape`) and wedge the channel over cosmetics. Absent roster = the original wire shape; older orchestrators stay compatible. Set a name with `display_name` in `[[agents]]`; names may collide across ids, and blanks are treated as absent.

**Claim 404 reads gone-or-never.** Redemption burns the claim *before* responding (so a concurrent double-fetch cannot both succeed). A claim that was minted, delivered, and then fetched once therefore returns the same `{"error":"no such claim"}` 404 body as one that never existed. A 404 must not be read as "never minted" — mint a fresh claim instead (or resume from persisted state). Fetching an outstanding claim a second time gets 410; so does an expired one.
=======
**Peer identity is provisioned, not inferred.** The redeem response also carries the relay's attestation of the channel's participant list — `participants` (token authors minus the reserved `orchestrator` auditor author), `participant` (which entry is the redeemer), and `peers` (the rest). `team join --from-claim-url` persists all of it to `bus.credentials.json` (0600) and prints the attested peer so the operator sees who provisioning named before any turn is exchanged. Every provisioning path names the peer explicitly: the operator side reads it from `bus.secret.json` + task meta, the joined side from the attested claim bundle, and the printed instructions state it outright. A participant runtime checks each wire turn's relay-attested `author` against the provisioned peer id *before* the turn validator — a non-provisioned author would otherwise be ignored as `not_participant` and the disagreement would surface only when transcripts diverged. On mismatch the runtime aborts loudly (the message is left uncommitted, so a restart re-aborts rather than stepping over it). Honest strength note: this is exactly as strong as the relay's authorship attestation — under the crash-faults-only trust model that is reliable; it catches provisioning/wire disagreement (a mis-provisioned channel), not a Byzantine relay.

**Hosting.** Reference relay on Fly; `team bus-serve` for local dev. The relay holds channel tokens, the auditor lease, and dedupe state; it sees ciphertext and metadata, never plaintext. `team bus-serve --data-dir <dir>` makes that state durable: a single SQLite file (bun:sqlite, WAL, `synchronous=FULL`) holds channels, SHA-256 token hashes — never raw tokens, so a stolen DB yields no working credentials — and the per-channel message log (ciphertext at rest, within the threat model). A restart then looks like a transient disconnect instead of a wipe. Deliberately transient: unredeemed claims die on restart (re-mint; post-restart mints take the participant's raw `token` in the request body since the relay can no longer re-derive it), long-poll waiters drop, and the auditor lease always boots unheld. Still one machine — durable is not multi-writer.
