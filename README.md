...
- `POST /c/<channel>/claim/<id>` — redeem a claim once: returns `{participant, participants, peers, token, channel_secret, channel, epoch, seat_id, seat_state}`, then the claim is dead (second redeem 404, expired 410). POST, not GET: HTTP defines GET as safe/idempotent, so link previews, mail scanners, and browser prefetchers issue one without malice and would burn the claim before its intended redeemer saw it. A `GET` here is non-destructive — 405 plus a hint, claim left intact. `seat_state` is `"vacant"` when the claim refilled a revoked seat, so a re-joining participant can tell it walked into an existing slot rather than a fresh one. `peers` is provisioning's attestation of the peer id — `team join --from-claim-url` persists the bundle to `bus.credentials.json` (0600), and the participant runtime aborts loudly if a wire turn arrives authored by anyone else.
